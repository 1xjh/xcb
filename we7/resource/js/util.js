...
util.url = function (action, querystring) {
	var app = getApp();
  var url = app.siteInfo.siteroot + action+ '?i=' + app.siteInfo.uniacid +'&v=' + app.siteInfo.version ;
	return url;
}
...
/**
	...
	@params option 弹出参数表，
	{
		url : 同微信,  'entry/wxapp/checkinfo'
		data : 同微信, {user_id:user_id}
		header : 同微信,
		method : 同微信, post
		success : 同微信,cc
		fail : 同微信,
...
util.request = function (option) {
	...
	var sessionid = wx.getStorageSync('userInfo').sessionid;   //拿本地缓存
...
//https://www.tiantiandj.com/xcx/index.php/index/Info/getTourist?....
	...
	if (sign) {
    url = url + "&sign=" + sign + "&token=" + wx.getStorageSync("token");
	}
	if (!url) {
		return false;
	}
	...
	wx.request({
    'url': url, //https://www.tiantiandj.com/xcx/index.php/index/Info/getTourist? &sign=" + sign + "token=" + getApp().siteInfo.token;
